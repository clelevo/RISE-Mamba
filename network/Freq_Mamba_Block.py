...
class SS2D_local(nn.Module):
    ...
    def local_scan(self, x, flip=False, column_first=False):
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        L = H * W
        K = 4
        x1 = self.local_scan(x)
        x2 = self.local_scan(x, column_first=True)
        x3 = self.local_scan(x, flip=True)
        x4 = self.local_scan(x, column_first=True, flip=True)
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        input = input.reshape(B*F, H, W, C).contiguous()
        ...
        # input.view(B*F, H, W, C).contiguous()
        x_final = x_final.view(B, F, C, H, W).contiguous()
        return x_final
...
if __name__ == '__main__':
    input = torch.randn(1, 10, 64, 320, 256).to('cuda')
    model = VSSBlock(hidden_dim=64).to('cuda')
    output = model(input)
    print(output.shape)