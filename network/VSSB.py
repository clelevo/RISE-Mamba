...
class VSSBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        input = input.reshape(B*F, H, W, C).contiguous()  # [B,H,W,C]
        x = self.ln_1(input)
        x = input * self.skip_scale + self.drop_path(self.self_attention(x))
        x = x * self.skip_scale2 + self.conv_blk(self.ln_2(x).permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3,
                                                                                                        1).contiguous()
...
class Wave_VSSBlock(nn.Module):
    def __init__(self,cs='dct_torch',hidden_dim=64):
        super(Wave_VSSBlock, self).__init__()
        self.cs = cs
        self.vssb = VSSBlock(hidden_dim=hidden_dim)
        if 'nodct' in cs:
...
if __name__ == '__main__':
    input = torch.randn(5, 1, 32, 64, 64).to('cuda')
    model = Wave_VSSBlock(cs='dct_torch',hidden_dim=32).to('cuda')
    output = model(input)
    print(output.shape)