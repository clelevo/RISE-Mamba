...
def	log_train_psnr_self_distillation(result, imsouce_class, loss, loss_out_M1F,loss_out_M2F,loss_M3_M1,loss_M3_M2,writer, epoch, idx, num_minibatches, training_params):
	# Log the scalar values
	psnr_train = psnr(result.detach(), imsouce_class.detach())

	print("[epoch {}][{}/{}] loss: {:1.4f} PSNR_train: {:1.4f} loss_out_M1F:{:1.4f} loss_out_M2F:{:1.4f} loss_M3_M1:{:1.4f} loss_M3_M2:{:1.4f} ".\
		  format(epoch+1, idx+1, num_minibatches, loss.item(), psnr_train,loss_out_M1F.item(),loss_out_M2F.item(),loss_M3_M1.item(),loss_M3_M2.item(),))

	writer.add_scalar('loss', loss.item(), training_params['step'])
	writer.add_scalars('PSNR on training val data', {'psnr_train': psnr_train}, \
					   training_params['step'])

def	log_train_psnr_self_distillation_pcs(result, imsouce_class, loss, loss_out_M1F,loss_out_M2F,loss_M3_M1,loss_M3_M2,loss_PCS,loss_PCS1,loss_PCS2,writer, epoch, idx, num_minibatches, training_params):
	# Log the scalar values
	psnr_train = psnr(result.detach(), imsouce_class.detach())

	print("[epoch {}][{}/{}] loss: {:1.4f} PSNR_train: {:1.4f} loss_out_M1F:{:1.4f} loss_out_M2F:{:1.4f} loss_M3_M1:{:1.4f} loss_M3_M2:{:1.4f} loss_PCS:{:1.4f} loss_PCS1:{:1.4f} loss_PCS2:{:1.4f} ".\
		  format(epoch+1, idx+1, num_minibatches, loss.item(), psnr_train,loss_out_M1F.item(),loss_out_M2F.item(),loss_M3_M1.item(),loss_M3_M2.item(),loss_PCS.item(),loss_PCS1.item(),loss_PCS2.item(),))
