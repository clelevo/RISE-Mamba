...
from train_common import resume_training, log_train_psnr, validate_and_log, save_model_checkpoint, log_train_psnr_self_distillation, log_train_psnr_self_distillation_pcs
import numpy as np
import random
from network.RISE_Mamba import RISE_Mamba
from network.STL_Loss import STL_loss
...
def main(**args):
    ...
    for epoch in range(start_epoch, args['epochs']):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            out_train, M1, M2, M3, M1_Mid_Feature, M2_Mid_Feature = model(imgn_train.reshape(N, F, C, H, W))

            imgn_train_ = rearrange(imgn_train, 'N F H W -> N F 1 H W')

            # Compute loss
            M1 = rearrange(M1, 'N F C H W ->  N (F C) H W')
            M2 = rearrange(M2, 'N F C H W ->  N (F C) H W')
            M3 = rearrange(M3, 'N F C H W ->  N (F C) H W')


            loss_PCS = STL_loss(out_train, imgn_train_)
            loss_PCS1 = STL_loss(M1_Mid_Feature, imgn_train_)
            loss_PCS2 = STL_loss(M2_Mid_Feature, imgn_train_)

            M1_Mid_Feature = rearrange(M1_Mid_Feature, 'n d c h w ->  n (d c) h w')
            M2_Mid_Feature = rearrange(M2_Mid_Feature, 'n d c h w ->  n (d c) h w')
            out_train = out_train.reshape(N, FC, H, W)

            loss_out_M1F = loss_calc(args, criterion_L2, img_train, M1_Mid_Feature)
            loss_out_M2F = loss_calc(args, criterion_L2, img_train, M2_Mid_Feature)
            loss_M3_M1   = loss_calc(args, criterion_L2, M3, M1)
            loss_M3_M2   = loss_calc(args, criterion_L2, M3, M2)

            loss = loss_calc(args, criterion_L2, img_train, out_train)

            LOSS_ALL = loss + 0.1 * loss_out_M1F + 0.1 * loss_out_M2F + 0.1 * loss_M3_M1 + 0.1 * loss_M3_M2 + 0.1 * loss_PCS + 0.1 * loss_PCS1 + 0.1 * loss_PCS2
            LOSS_ALL.backward()
            ...
            if training_params['step'] % args['save_every'] == 0:
                log_train_psnr_self_distillation_pcs(out_train,
                               img_train,
                               loss,0.1 * loss_out_M1F,0.1 * loss_out_M2F,0.1 * loss_out_M1F,0.1 * loss_out_M1F,0.1 * loss_PCS, 0.1 * loss_PCS1, 0.1 * loss_PCS2,
                               writer,
                               epoch,
                               i,
                               num_minibatches,
...
if __name__ == "__main__":
    ...
    #Training parameters
    parser.add_argument("--batch_size", type=int, default=6,   # default = 6
    ...
    parser.add_argument("--save_all", default="all",
    ...
    parser.add_argument("--temp_patch_size", "--tp", type=int, default=5, help="Temporal patch size")   # 5
